...
const Hyperspeed = ({
  ...
  useEffect(() => {
    ...
    class App {
      ...
      init() {
        if (!this.composer || !this.scene || !this.road) return;
        this.initPasses();
      ...
      tick() {
        if (this.disposed || !this || !this.renderer || !this.composer) return;
    ...
    // Add a ResizeObserver to start the app when it gets proper dimensions
    const resizeObserver = new ResizeObserver(() => {
      if (!hyperspeed.current || !appRef.current) return;
      const container = hyperspeed.current;
      if (container.offsetWidth > 0 && container.offsetHeight > 0 && !appRef.current.composer) {
        appRef.current.loadAssets().then(() => appRef.current.init());
      }
    });
    if (hyperspeed.current) {
      resizeObserver.observe(hyperspeed.current);
    }
