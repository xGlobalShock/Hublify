...
`;
    ...
    class Simulation {
      ...
      calcSize() {
        if (Common.width === 0 || Common.height === 0) return;
    ...
    // Defer starting until we have dimensions
    ...
    const io = new IntersectionObserver(
      entries => {
        ...
        if (isVisible && !document.hidden) {
          // Only start if we have dimensions
          if (!startedRef.current && Common.width > 0 && Common.height > 0) {
    ...
    const ro = new ResizeObserver(() => {
      if (!webglRef.current || !container) return;
      try {
        if (resizeRafRef.current) cancelAnimationFrame(resizeRafRef.current);
        resizeRafRef.current = requestAnimationFrame(() => {
          if (!webglRef.current || !container || !container.parentElement) return;
          webglRef.current.resize();
          
          // Start rendering if not already started and we now have dimensions
          if (!startedRef.current && Common.width > 0 && Common.height > 0 && isVisibleRef.current && !document.hidden) {
            webglRef.current.start();
            startedRef.current = true;
          }
        });
  ...
  return <div ref={mountRef} className={`liquidether-container ${className || ''}`} style={style} />;
}
